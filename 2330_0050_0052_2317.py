...
st.markdown("Compare **0052 vs 2317 vs 2330 vs 0050**")
...
if st.button("🚀 Run Simulation"):
    ...
    with st.spinner(f"Downloading market data from {start_date} to {end_date}..."):
        try:
            tickers = ["0052.TW", "0050.TW", "2330.TW", "2317.TW"]
            ...
            # 1. 0052 (Tech ETF)
            p_start_052 = data["0052.TW"].iloc[0]
            p_end_052   = data["0052.TW"].iloc[-1]
            shares_052  = capital / p_start_052
            final_052   = shares_052 * p_end_052
            ret_052     = ((final_052 - capital) / capital) * 100
            ...
            col1.metric("0052 (Tech ETF)", f"${int(final_052):,}", f"{ret_052:.2f}%")
            ...
            # --- THE VERDICT ---
            winner_val = max(final_052, final_2317, final_2330, final_050)
            
            if winner_val == final_052:
                st.balloons()
                st.success("🏆 **WINNER: 0052 (Fubon Tech)**")
